...
PAYARA_HOME="${PAYARA_HOME:-/opt/payara/appserver}"
ASADMIN="${PAYARA_HOME}/bin/asadmin"
DOMAIN="${DOMAIN:-domain1}"
DOMAIN_DIR="${PAYARA_HOME}/glassfish/domains/${DOMAIN}"

# Start domain in background to run admin commands
"$ASADMIN" start-domain "$DOMAIN"

# Ensure MySQL JDBC driver is available at domain level
mkdir -p "${DOMAIN_DIR}/lib"
if [ -f "${PAYARA_HOME}/glassfish/modules/mysql-connector-j.jar" ]; then
  cp "${PAYARA_HOME}/glassfish/modules/mysql-connector-j.jar" "${DOMAIN_DIR}/lib/mysql-connector-j.jar" || true
fi
